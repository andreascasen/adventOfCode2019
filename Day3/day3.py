def positions_are_equal(pos1, pos2):
	return pos1['x'] == pos2['x'] and pos1['y'] == pos2['y']

def calc_distance_to_start(current_position):
	return abs(current_position['x']) + abs(current_position['y'])

# parse_path_section
#
# Parses a single path section and returns an object
# containing the following properties:
# 	newSteps - Array of the steps taken on the path
#	newCurrentPosition - Position of the wire at the end of the path.
def parse_path_section(pathSection, currentPosition):
	direction = pathSection[0]
	distance = int(pathSection[1:])
	...
	while i < distance:
		if direction == 'R':
			new_current_position['x'] = new_current_position['x'] + 1
			new_steps.append({
				'x': new_current_position['x'],
				'y': new_current_position['y'],
				'distanceToStart': calc_distance_to_start(new_current_position)
			})
		elif direction == 'L':
			new_current_position['x'] = new_current_position['x'] - 1
			new_steps.append({
			...
			new_current_position['y'] = new_current_position['y'] + 1
			new_steps.append({
				'x': new_current_position['x'],
				'y': new_current_position['y'],
				'distanceToStart': calc_distance_to_start(new_current_position)
			...
			raise Exception('Invalid path direction')

		i += 1

	return { 'newSteps': new_steps, 'newCurrentPosition': new_current_position }

def map_wire_positions(wire_path):
	current_position = { 'x': 0, 'y': 0 }
	steps = []

	for path_section in wire_path:
		single_result = parse_path_section(path_section, current_position)
		current_position = single_result['newCurrentPosition']
		for single_step in single_result['newSteps']:
			steps.append(single_step)
	
	sorted_steps = sorted(steps, key=lambda k: k['distanceToStart'])
	return sorted_steps

def find_closest_intersection(wire_1, wire_2):
	wire_1_positions = map_wire_positions(wire_1)
	wire_2_positions = map_wire_positions(wire_2)
	closest_intersection_distance = 0

	wire_1_current_position: { 'x': 0, 'y': 0 }
	wire_2_current_position: { 'x': 0, 'y': 0 }

	top_index = 0
	intersection_found = False
	while top_index < len(wire_1_positions) and intersection_found is not True:
		current_position_1 = wire_1_positions[top_index]
		inside_index = 0

		while inside_index < len(wire_2_positions) and intersection_found is not True:
			current_position_2 = wire_2_positions[inside_index]

			if positions_are_equal(current_position_1, current_position_2):
				closest_intersection_distance = current_position_1['distanceToStart']
